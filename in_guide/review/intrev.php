...
<div class="container well">
 ...
 <br><br>
  ...
  $q3 = mysql_query("select * from intern_student natural join internal_guide where int_id='".$seloption2."'")or die ('Query is invalid: ' . mysql_error());
  $q2 = mysql_query("select * from intern_student natural join external_guide where int_id='".$seloption2."'")or die ('Query is invalid: ' . mysql_error());
  
  while ($row2 = mysql_fetch_array($q3)) {
  
  	
  	echo '<b>Name:</b> '.$row2['int_name'].'<br>'.'<b>Reg ID:</b> '.$row2['int_id'].'<br>';echo '<b>Type: </b>Internship Student<br>';
  	echo '<b>Project Name: </b>'.$row2['int_pro'].'<br><b>Project Domain:</b> '.$row2['int_domain'].'<br><b>Project Abstract:</b> '.$row2['abstract'];
  	echo '<br><b>VIT Guide:</b> '.$row2['g_name'];
  	echo '<input type="hidden" class="form-control" id="snameint" name="snameint" value="'.$row2['int_name'].'">';
  	echo '<input type="hidden" class="form-control" id="sidint" name="sidint" value="'.$row2['int_id'].'">';
  	
  }
  while ($row22 = mysql_fetch_array($q2)) {
  
  
  	echo '<br><b>Company Guide:</b> '.$row22['e_name'].'<br><b>Company:</b> '.$row22['e_company'].'<br>';
  
  	//echo "	<option name='".$row['int_id']."' value='".$row['int_id']."'> ".$row['int_name']." </option>";
  }
  
  echo '<br><h3>Status : &nbsp;&nbsp;&nbsp;';
  $q21 = mysql_query("select * from mainreview1statusintern where int_id='".$seloption2."'")or die ('Query is invalid: ' . mysql_error());
  ...
  $q212 = mysql_query("select * from mainreview1 where s_id='".$seloption2."'")or die ('Query is invalid: ' . mysql_error());
...
<label class="radio-inline"><input type="radio" name="yesno" value="No">No</label>
  ...
  if(isset($_POST['submit'])){
  	...
  	values ('$sname','$sid','$gid','$gname','$q1','$q2','$q3','$q4','$q5','$b','$c','intern','Posted')" ;
